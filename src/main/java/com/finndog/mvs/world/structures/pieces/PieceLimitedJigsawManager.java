...
public class PieceLimitedJigsawManager {
    ...
    public static <C extends FeatureConfiguration> Optional<PieceGenerator<C>> assembleJigsawStructure(
        ...
        if(startPool == null || startPool.size() == 0) {
            MVSMain.LOGGER.warn("Moogs Voyager Structures: Empty or nonexistent start pool in structure: {}  Crash is imminent", structureID);
            throw new RuntimeException("Moogs Voyager Structures: Empty or nonexistent start pool in structure: " + structureID + " Crash is imminent");
        }
                ...
                if (attempts == 100) {
                    MVSMain.LOGGER.error(
                            """
                                    ...
                                    -------------------------------------------------------------------
                                    Moogs Voyager Structures: Failed to create valid structure with all required pieces starting from this pool file: {}. Required pieces failed to generate the required amount are: {}
                                      ...
                                      Please report the issue to Moogs Voyager Structures' dev with latest.log file if the structure is not cut off by world min/max y build thresholds.
        ...
        public void generatePiece(PoolElementStructurePiece piece, MutableObject<BoxOctree> boxOctree, int minY, int depth, boolean doBoundaryAdjustments, LevelHeightAccessor heightLimitView) {
            ...
            for (StructureTemplate.StructureBlockInfo jigsawBlock : pieceJigsawBlocks) {
                ...
                if (!(poolOptional.isPresent() && (poolOptional.get().size() != 0 || Objects.equals(jigsawBlockPool, Pools.EMPTY.location())))) {
                    MVSMain.LOGGER.warn("Moogs Voyager Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockPool, pieceBlueprint instanceof SinglePoolElement ? ((SinglePoolElementAccessor) pieceBlueprint).repurposedstructures_getTemplate().left().get() : "not a SinglePoolElement class");
                    continue;
                }
                ...
                if (!(fallbackOptional.isPresent() && (fallbackOptional.get().size() != 0 || Objects.equals(jigsawBlockFallback, Pools.EMPTY.location())))) {
                    MVSMain.LOGGER.warn("Moogs Voyager Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockFallback, pieceBlueprint instanceof SinglePoolElement ? ((SinglePoolElementAccessor) pieceBlueprint).repurposedstructures_getTemplate().left().get() : "not a SinglePoolElement class");
                    continue;
                }
