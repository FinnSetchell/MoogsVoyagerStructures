...
//                RegistryCodecs.entryList(RegistryKeys.STRUCTURE_SET, StructureSet.CODEC).fieldOf("other_set").forGetter(AdvancedRandomSpread.SuperExclusionZone::otherSet),
//                Codec.intRange(1, 16).fieldOf("chunk_count").forGetter(SuperExclusionZone::chunkCount)
//        ).apply(builder, SuperExclusionZone::new));
//
//        boolean isPlacementForbidden(ChunkGeneratorStructureState chunkGeneratorStructureState, NoiseConfig noiseConfig, long seed, int x, int z) {
//            for (RegistryEntry<StructureSet> registryEntry : this.otherSet) {
//                if (chunkGeneratorStructureState.hasStructureChunkInRange(registryEntry, noiseConfig, seed, x, z, this.chunkCount)) {
//                    return true;
//                }
//            }
//
