...
public class NonFreezingMapRegistry<T> extends WritableRegistry<T> {
    ...
    private Holder<T> registerMapping(int p_205858_, ResourceKey<T> p_205859_, T p_205860_, Lifecycle p_205861_, boolean p_205862_) {
        ...
        if (this.customHolderProvider != null) {
            ...
            reference = this.byKey.computeIfAbsent(p_205859_, (p_205927_) -> Holder.Reference.createStandAlone(this, p_205927_));
        }
    ...
    public Registry<T> freeze() {
        List<ResourceLocation> list = this.byKey.entrySet().stream()
                .filter((p_211055_) -> !p_211055_.getValue().isBound())
                .map((p_211794_) -> p_211794_.getKey().location()).sorted().toList();
        if (!list.isEmpty()) {
            throw new IllegalStateException("Unbound values in registry " + this.key() + ": " + list);
        } else {
            if (this.intrusiveHolderCache != null) {
                List<Holder.Reference<T>> list1 = this.intrusiveHolderCache.values().stream()
                        .filter((p_211809_) -> !p_211809_.isBound()).toList();
                if (!list1.isEmpty()) {
    ...
    public Holder.Reference<T> createIntrusiveHolder(T p_205915_) {
        ...
        } else if (this.intrusiveHolderCache != null) {
            return this.intrusiveHolderCache.computeIfAbsent(p_205915_,
                    (p_211813_) -> Holder.Reference.createIntrusive(this, p_211813_));
        } else {
    ...
    public void bindTags(Map<TagKey<T>, List<Holder<T>>> p_205875_) {
        ...
        if (!set.isEmpty()) {
            LOGGER.warn("Not all defined tags for registry {} are present in data pack: {}", this.key(), set.stream()
                    .map((p_211811_) -> p_211811_.location().toString()).sorted().collect(Collectors.joining(", ")));
        }
