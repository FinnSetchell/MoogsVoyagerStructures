...
import com.finndog.mvs.mixin.structures.ListPoolElementAccessor;
...
import com.finndog.mvs.mixin.structures.StructureTemplateManagerAccessor;
...
public final class PoolAdditionMerger {
    ...
    public static void mergeAdditionPools() {
        ServerLifecycleEvents.SERVER_STARTING.register((MinecraftServer minecraftServer) -> {
            ResourceManager resourceManager = ((StructureTemplateManagerAccessor) minecraftServer.getStructureManager()).mvs_getResourceManager();
            ...
            for (JsonElement jsonElement : entry.getValue()) {
                try {
                    ...
                    MVSMain.LOGGER.error("""
                            ...
                            MVS: Pool Addition json failed to be parsed.
    ...
    private static void checkIfPieceExists(AdditionalStructureTemplatePool feedingPool, StructureTemplateManager structureTemplateManager, ResourceLocation pieceRL) {
        ...
        try {
            ...
            if (inputstream.available() == 0 || inputstream.read(new byte[1]) == -1) {
                MVSMain.LOGGER.error("(MVS POOL MERGER) Found an entry in {} that points to the non-existent nbt file called {}", feedingPool.getName(), pieceRL);
            }
            inputstream.close();
        }
    ...
    private static void logBadData(ResourceLocation poolPath, String messageString) {
        MVSMain.LOGGER.error("(MVS POOL MERGER) Failed to parse {} additions file. Error is: {}", poolPath, messageString);
    }
        ...
        public AdditionalStructureTemplatePool(ResourceLocation resourceLocation, ResourceLocation resourceLocation2, List<ExpandedPoolEntry> rawTemplatesWithConditions) {
            super(resourceLocation, resourceLocation2, rawTemplatesWithConditions.stream().filter(triple -> {
                if(triple.condition().isPresent()) {
                    Optional<Supplier<Boolean>> optionalSupplier = JSONConditionsRegistry.MVS_JSON_CONDITIONS_REGISTRY.getOptional(triple.condition.get());
                    if(optionalSupplier.isPresent()) {
                        return optionalSupplier.get().get();
                    }
                    else {
                        MVSMain.LOGGER.error("MVS Error: Found {} entry has a condition that does not exist. Extra info: {}", resourceLocation, resourceLocation2);
                    }
                }
                return true;
