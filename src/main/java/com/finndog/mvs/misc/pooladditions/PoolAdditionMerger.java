    ...
    private static void checkIfPieceExists(AdditionalStructureTemplatePool feedingPool, StructureTemplateManager structureTemplateManager, ResourceLocation pieceRL) {
        ...
        try {
            ...
            if (inputstream.available() == 0 || inputstream.read(new byte[1]) == -1) {
                MVSMain.LOGGER.error("(MVS POOL MERGER) Found an entry in {} that points to the non-existent nbt file called {}", feedingPool.name, pieceRL);
            }
            inputstream.close();
        }
        ...
        public AdditionalStructureTemplatePool(ResourceLocation name, Holder<StructureTemplatePool> fallback, List<ExpandedPoolEntry> rawTemplatesWithConditions) {
            super(fallback, rawTemplatesWithConditions.stream().filter(triple -> {
                if(triple.condition().isPresent()) {
                    Supplier<Boolean> supplier = MVSConditionsRegistry.MVS_JSON_CONDITIONS_REGISTRY.lookup().get(triple.condition.get());
                    if(supplier != null) {
                        return supplier.get();
                    }
                    else {
